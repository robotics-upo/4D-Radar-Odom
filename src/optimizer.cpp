...
public:
    ...
    GraphSlam() : Node("graph_slam"), stop_processing_(false) {
        ...
        imu_subscriber_ = create_subscription<sensor_msgs::msg::Imu>(
            "/vectornav/imu", 1024, [this](const sensor_msgs::msg::Imu::SharedPtr msg) {
                {
                    std::lock_guard<std::mutex> lock(queue_mutex_);
        ...
        point_cloud_subscriber_ = create_subscription<sensor_msgs::msg::PointCloud2>(
            "/filtered_points", 300, [this](const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
                {
                    std::lock_guard<std::mutex> lock(queue_mutex_);
...
private:
    ...
    bool isTwistBeforeImu(const geometry_msgs::msg::TwistWithCovarianceStamped::SharedPtr& twist_msg,
    ...
    bool isImuBeforePointCloud(const sensor_msgs::msg::Imu::SharedPtr& imu_msg,
    ...
    bool validatePose(const tf2::Transform& pose) {
        return !(std::isnan(pose.getOrigin().getX()) || std::isnan(pose.getOrigin().getY()) || std::isnan(pose.getOrigin().getZ()) ||
                 std::isnan(pose.getRotation().x()) || std::isnan(pose.getRotation().y()) || std::isnan(pose.getRotation().z()) || std::isnan(pose.getRotation().w()));
    }
